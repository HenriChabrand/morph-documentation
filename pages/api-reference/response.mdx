...
<Callout type='info'>
Make sure to replace placeholder values (e.g., `{ACTION_TYPE}`, `{ACTION_NAME}`) with actual values where necessary. The type of the content and color for the status should be chosen from the prescribed set of values.
</Callout>
